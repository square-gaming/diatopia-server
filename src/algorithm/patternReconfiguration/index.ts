...
import {BlocksType, StructuresType} from '../../types';
...
export function configureRule(
  ...
  function isStructures(
    ...
    return blocks.every(block => block instanceof Structure || block === null);
  }
  ...
  } else if (isStructures(adjacents) && source instanceof Door) {
    return door(adjacents, source);
  } else if (isFloors(adjacents) && source instanceof Floor) {
