...
class Server {
    ...
    public listen() {
        console.log(`WebSocket listening on ws://${this.wss.options.host}:${this.wss.options.port}`);
        this.wss.addListener('connection', function connection(this: Server, ws: WebSocket, req: http.IncomingMessage) {
    ...
    private websocketSetup(ws: WebSocket, req: http.IncomingMessage) {
        const uid = req.headers.cookie ? req.headers.cookie.split('=')[1].slice(0, -1) : '';
        
        console.log(`Player ${uid} connects`);

        ws.on('message', function (this: Server, message: string) {
            const uid = req.headers.cookie? req.headers.cookie.split('=')[1].slice(0, -1) : '';
            const data: {type: string; payload: any} = JSON.parse(message);

            if (uid) {
                MessageHandle[data.type](uid, data, this.manager, ws, this);
            } else {
                console.error('Someone sending message without cookie.');
            }
        }.bind(this));

        ws.on('close', function (this: Server) {
